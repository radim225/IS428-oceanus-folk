...
Three CSV files were derived from the master `mc1_nodes.csv` and `mc1_edges.csv` files using a Python preprocessing script (`Data/preprocess_mc1.py`).

`task3_notable_works.csv` (1,973 rows) was constructed by filtering all nodes where `notable = True` and `notoriety_date` is not empty, then joining `PerformerOf` edges to resolve the performing artist for each charted work. This produces one row per (work, performer) pair, retaining `genre`, `release_date`, `notoriety_date`, and `performer_type`.

`task3_artist_notoriety.csv` (1,664 rows) aggregates the above to one row per artist, computing four summary metrics: `total_notable_works` (count of charted works), `first_notoriety_date` (earliest chart appearance), `latest_notoriety_date` (most recent chart appearance), and `years_active_span` (difference between earliest and latest `release_date` across all their works). A boolean flag `is_oceanus_folk_artist` is set to `True` if any of the artist's genres contain "Oceanus Folk". Of the 1,664 artists, 56 are flagged as Oceanus Folk artists, and 14 of those have two or more notable works.

`task3_of_candidates.csv` (84 rows) filters `task3_notable_works` to Oceanus Folk artists only and joins the four summary metrics from `task3_artist_notoriety`. This file contains one row per notable work per Oceanus Folk artist, enabling both work-level and artist-level analysis within the same Tableau data source.
...
![Dashboard 1 — "Who are the Candidates?": OF Artist Ranking (top-left), Genre Mix by Candidate (top-right), Breakthrough Profile scatter plot (bottom)](images/task3/task3_Who_are_the_candidates_.png){width=100%}
...
![Dashboard 2 — "Who's Next?": Three Artist Comparison (top-left), Notoriety Timeline (bottom-left), radar chart and data table (right)](images/task3/task3_whos_next_dashboard_2.png){width=100%}
